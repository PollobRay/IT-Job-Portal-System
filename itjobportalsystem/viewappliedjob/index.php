<?php
    ...
    if(!isset($_SESSION['email']) || $_SESSION['login']!='users')
    {
    ...
    $q="select * from jobs, applyjob where jobs.job_serial=applyjob.job_serial_no AND applyjob.user_email='$email'; ";
...
<body>
<script>
    function myFunction(jtitle,company,description,jno,status)
    {
        ...
        document.getElementById("jstatus").value = status; 
   ...
   <div class="secondDiv" style="height:100%;">
         <div class="inner-box" style="width:60%;  margin: right 10px; padding: right 20px;">
            ...
            <div class="card-front" style="overflow-y:scroll;  overflow-x:scroll;  buttom 100px;">
                ...
                ?>
                <h2 style="cursor:pointer;"  onclick="myFunction('<?php echo $title[$i]; ?>','<?php echo $company[$i]; ?>','<?php echo $description[$i]; ?>','<?php echo $job_serial[$i]; ?>','<?php echo $applied_status[$i]; ?>')"><?php echo $title[$i]; ?></h2>
         ...
         <div class="sidepanel" style="width:40%; overflow-x:scroll;  padding: right 20px;">
                <form action="job/index.php" method="post">
                    <input id="jstatus" name="jstatus" type="text"  style="visibility: hidden;" >
