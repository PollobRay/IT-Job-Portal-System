<?php
    ...
    $q="SELECT * FROM applyjob ,users WHERE applyjob.user_email=users.email && applyjob.job_serial_no=$serial && applyjob.status='selected'; ";
    ...
    while($row=mysqli_fetch_array($result,MYSQLI_ASSOC))
    {
        ...
        $email[]=$row["email"];
        $cnt_no[]=$row["cnt_no"];
        $applied_serial[]=$row["applied_job_serial_no"];
        $len ++;
    }
...
<body>
<script>
    function myFunction(fname,lname,email,cntno,appserial)
    {
        ...
        document.getElementById("jaserial").value = appserial; 
...
<div claas="main">
   ...
   <div class="secondDiv" style="height:100%;">
         <div class="inner-box" style="width:60%;  margin: right 10px; padding: right 20px;">
            <br><br>
            <h1 style="color:red; text-align:center;">Selected Applicant List</h1>
            <hr>
            <br><br>
            <div class="card-front" style="overflow-y:scroll;  overflow-x:scroll;  buttom 100px;">
                ...
                ?>
                <h2 style="cursor:pointer;"  onclick="myFunction('<?php echo $fname[$i]; ?>','<?php echo $lname[$i]; ?>','<?php echo $email[$i]; ?>','<?php echo $cnt_no[$i]; ?>','<?php echo $applied_serial[$i]; ?>')"><?php echo $fname[$i]." ".$lname[$i]; ?></h2>
         ...
         <div class="sidepanel" style="width:40%; overflow-x:scroll;  padding: right 20px;">
                <form action="user/index.php" method="post">
                    <input id="jaserial" name="jaserial" type="text"  style="visibility: hidden;" >
